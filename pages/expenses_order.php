<?php 
    ...
    if($_SERVER['REQUEST_METHOD'] === 'POST'){
                ...
                while($row = mysqli_fetch_array($query))
                {
                    echo "
                        <div class='col-sm-6 col-lg-4 mb-4'>
                            <div class='card expense_card $row[Month]' id='$row[ID]' data-month='$row[Month]'>
        ...
        if($order === 'desc'){
            ...
            $sql = "SELECT * FROM `expense` WHERE Name = '$username' ORDER BY `Date_billing` ASC , `Time_create` DESC";
            OrderSql($con, $sql);
        }
    }
?>